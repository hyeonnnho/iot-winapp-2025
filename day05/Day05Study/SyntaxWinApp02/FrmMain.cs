namespace SyntaxWinApp02
{
    public partial class FrmMain : Form
    {
        ...
        private void BtnCheck_Click(object sender, EventArgs e)
        {
            ...
            // delegate = 람다식 (x, y) => x + y
            Func<int, int, int> add2 = (x, y) => x + y;
            TxtLog.Text += result + "\r\n";
            TxtLog.Text += add2(10, 30) + "\r\n";

            // SayHello 함수 생성 대신
            Action<string> sayHello = (name) => MessageBox.Show($"안녕, {name}", "메시지", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
    ...
    public class Box<T> // where T : struct
    {
        ...
        public void Show()
        {
            Console.WriteLine($"Box의 값 : {Value}", "Box값", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }
}

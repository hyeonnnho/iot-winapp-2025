namespace SyntaxWinApp03
{
    public partial class FrmMain : Form
    {
        ...
        {
            //string filePath = "sample.txt"; // 파일저장
            // 3. SaveFileDialog 사용
            ...
            if (DlgSave.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    string ext = Path.GetExtension(fullPath);   // .rtf, .txt 가져오기

                    if (ext == ".rtf")
                        RtbNote.SaveFile(fullPath, RichTextBoxStreamType.RichText);
                    else if (ext == ".txt")
                        RtbNote.SaveFile(fullPath, RichTextBoxStreamType.PlainText);
                    MessageBox.Show("파일이 저장되었습니다.", "파일 저장", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void BtnLoad_Click(object sender, EventArgs e)
        {
            ...
            // 3. OpenFileDialog 사용
            DlgOpen.FileName = string.Empty; // == ""; 와 같음
            ...
            //    else
            //    {
            //        MessageBox.Show("파일이 존재하지 않습니다.", "파일 읽기", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    }
            //}
            //catch (Exception ex)
            //{
            //    MessageBox.Show($"읽기 실패 : {ex.Message}", "파일 저장", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //}
        }
